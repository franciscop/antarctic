...
The domain should not include the protocol or path. Pass the client secret for confidential clients.
...
`validateAuthorizationCode()` will either return an [`OAuth2Tokens`](/reference#oauth2tokens), or throw one of [`OAuth2RequestError`](/reference#oauth2requesterror), [`ArcticFetchError`](/reference#arcticfetcherror), [`UnexpectedResponseError`](/reference#unexpectedresponseerror), or [`UnexpectedErrorResponseBodyError`](/reference#unexpectederrorresponsebodyerror). Auth0 returns an access token, the access token expiration, and a refresh token.
...
```ts
const response = await fetch("https://xxx.auth0.com/userinfo", {
	headers: {
